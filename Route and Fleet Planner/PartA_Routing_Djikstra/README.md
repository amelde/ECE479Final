...
- Computes routes using Dijkstra shortest path search
...
Use the program in this order for the cleanest demo:
...
This implementation demonstrates a graph-based route and fleet planner for FOODIE. The campus is modeled as a weighted graph, robots are stored with capacity and battery constraints, and pending delivery orders are assigned to the cheapest feasible robot. Dijkstra shortest-path search is used for routing, and the system automatically replans all assigned routes whenever a path is blocked or reopened.
